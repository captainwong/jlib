...
inline std::string toString(const unsigned char* data, size_t len,
                            ToStringOption option = ToStringOption::TRY_IS_PRINT_FIRST,
                            bool show_x_for_hex = true,
                            bool show_space_between_hex = false) {
    return toString((const char*)data, len, option, show_x_for_hex, show_space_between_hex);
}

inline std::string toString(uint8_t b, bool show_x_for_hex = true) {
    return toString(&b, 1, ToStringOption::ALL_CHAR_AS_HEX, show_x_for_hex);
}

inline std::string toString(uint16_t b2, bool show_x_for_hex = true) {
    return toString((const char*)&b2, 2, ToStringOption::ALL_CHAR_AS_HEX, show_x_for_hex);
}

inline std::string toString(uint32_t b4, bool show_x_for_hex = true) {
    return toString((const char*)&b4, 4, ToStringOption::ALL_CHAR_AS_HEX, show_x_for_hex);
}

inline std::string toString(uint64_t b8, bool show_x_for_hex = true) {
    return toString((const char*)&b8, 8, ToStringOption::ALL_CHAR_AS_HEX, show_x_for_hex);
}

inline std::string byte2bits(uint8_t byte, bool with_prefix = true) {
    std::string bits = with_prefix ? "b" : "";
    uint8_t mask = 0x80;
    while (mask) {
        bits += (byte & mask) ? '1' : '0';
        mask >>= 1;
...
inline std::string word2bits(uint16_t word, bool with_prefix = true) {
    std::string str = with_prefix ? "b" : "";
    return str + byte2bits((uint8_t)(word >> 8), false) + " " + byte2bits((uint8_t)(word & 0xFF), false);
}

inline std::string dword2bits(uint32_t dword, bool with_prefix = true) {
    std::string str = with_prefix ? "b" : "";
